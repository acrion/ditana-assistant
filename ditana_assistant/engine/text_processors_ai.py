...
from ditana_assistant.engine import input_analyzers_ai, text_processors_regex
...
def translate_from_defined_language(
...
def generate_factual_query(
    request: str, messages: List[Dict[Literal["role", "content"], str]]
) -> str:
    """
...
It is important that you only ask the question and do not answer the above request directly."""
    ...
    factual_query = ConversationManager(messages).process_input(
        request_to_generate_factual_query
    )[0]

    OutputManager.print_formatted("factual contextual query", factual_query)

    return factual_query


def generate_critical_question(
    preliminary_assistant_answer: str,
    ...
    return conversation.process_input(
        "Please formulate a critical question regarding how I answered the above. Just issue the question itself, without any comments."
    )[0]


def generate_systematic_query(
    request: str, messages: List[Dict[Literal["role", "content"], str]]
) -> Tuple[str, str]:
    """
...
It is crucial that you do not answer anything, but only issue a short, self-contained question."""
    ...
    systematic_query = ConversationManager(messages).process_input(
        request_to_generate_factual_query
    )[0]
...
def generate_sub_prompts(
    prompt: str, messages: List[Dict[Literal["role", "content"], str]]
) -> List[str]:
    """
...
It is crucial that you do not answer anything, but only issue the two sub-prompts."""
    ...
    sub_prompts_str = ConversationManager(messages).process_input(
        request_to_generate_sub_prompts
    )[0]
...
It is important that you only list the facts and do not answer the above request directly."""
    ...
    OutputManager.print_formatted("fact list", fact_list)
...
```"""
    ...
    conversation = ConversationManager(messages)
    answer_to_socratic_question = conversation.process_input(
        request_to_generate_fact_list
    )[0]

    OutputManager.print_formatted("socratic_method", answer_to_socratic_question)
